...
def test_migrations_apply_and_are_idempotent():
    async def main() -> None:
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as d:
            ...
            # Migration 015 additions: Stored Qi pool + pill seeds
            for col in ("stored_qi_current", "stored_qi_max",
                        "stored_qi_max_bonus", "stored_qi_regen_bonus"):
                assert col in cult_cols
            sq_pills = await db.fetchone(
                "SELECT COUNT(*) AS c FROM item_templates WHERE item_type='Pill'"
                " AND name LIKE 'Stored Qi %'"
            )
            assert sq_pills["c"] == 3

            # Migration 016: dao_law mastery_effect re-keyed to the 5 ranks
            space = await db.fetchone(
                "SELECT mastery_effect FROM dao_laws WHERE name='Law of Space'"
            )
            ...
            # Migration 019 additions: sect array burst cooldown column
